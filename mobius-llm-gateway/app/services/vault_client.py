...
def initialize_vault_kubernetes():
    """
    Explicitly initialize Kubernetes configuration from Vault.
    ...
    try:
        ...
        logger.warning("Failed to load IN-CLUSTER Kubernetes config")
        if DISABLE_VAULT:   
            logger.info("Vault is disabled, skipping Kubernetes initialization.")
            return

        try:
            kubeconfig_paths = create_kubeconfig_from_vault()
            logger.info("✅ Kubeconfig successfully configured from Vault")
            ...
            logger.info(f"kubectl context set: {result.stdout.strip()}")
...
def get_vault_secret(path: str, key: str) -> str:
    ...
    """
    with tracer.start_as_current_span("vault.get_secret") as span:
        span.set_attribute("vault.path", path)
        span.set_attribute("vault.key", key)
...
def get_k8s_secret_data() -> Optional[Dict[str, Any]]:
    ...
    """
    with tracer.start_as_current_span("vault.get_k8s_secret_data") as span:
        span.set_attribute("vault.path", VAULT_K8S_SECRET_PATH)
        client_obj = get_vault_client()
        if not client_obj:
            return None
...
def create_kubeconfig_from_vault() -> Dict[str, str]:
    ...
    with tracer.start_as_current_span("vault.create_kubeconfig") as span:
        span.set_attribute("vault.path", VAULT_BASE_PATH)
        try:
...
def get_hf_token_b64() -> str:
    ...
    """
    with tracer.start_as_current_span("vault.get_hf_token"):
