...
# ---- chain-run queue (DITA-specific) ----
async def enqueue_chain_run(run_id: str) -> None:
    await get_redis().rpush(_settings.chain_queue_name, run_id)


async def dequeue_chain_run(timeout: int = 5) -> str | None:
    res = await get_redis().blpop([_settings.chain_queue_name], timeout=timeout)
    if res is None:
        return None
    _, rid = res
    return rid
